...
<html>
  ...
  <body>
    ...
    <section id='sotd'>
      ...
      <section>
        ...
        <section>
          <h3>9 May 2015</h3>
          <ul>
            <li>Marked inline comments as deprecated, pending removal in v1.0.</li>
            <li>Added nested arrays as a required feature.</li>
          </ul>
        </section>
        <section>
          <h3>23 April 2015</h3>
    ...
    <section>
      ...
      <p>
        The value of <code>key</code> should be <code>value\n:end</code> (backslash removed):
      </p>
      <pre><code>
        key: value
        \:end
    ...
    <section>
      ...
      <section>
        ...
        <p>
          This shoud produce the following structure: <code>{"array": [{"subarray": [{"key": value"}]}]}</code>.
        </p>
        <p>
          Nested array keys can be mixed freely with normal key/value pairs. But note that subarrays must be closed with empty brackets (<code>[]</code>) in order to "return" to the parent element.
        </p>
        <pre><code>
        [array]
        key: Depth 1
